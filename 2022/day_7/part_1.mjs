...
class FileSystem {
	...
	findSumInThreshold(threshold, node = this.head) {
		const currsize = node.size < threshold && node.dir ? node.size : 0;
		const sumChildren = node.children
			?.map(x => this.findSumInThreshold(threshold,x));
		return currsize + sumChildren?.reduce((a, x) => a + x, 0);
	}
}
...
console.log(fs.findSumInThreshold(100000));
